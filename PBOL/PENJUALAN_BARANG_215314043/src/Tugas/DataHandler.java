...
public class DataHandler {
    ...
    public void setAllPenjualan(int kode, int jumlah_stock, int jumlah_beli, String nama, double harga, int id_jual) throws SQLException {
        stmt2 = conn.createStatement();
        query2 = "insert into penjualan (nama, harga, jumlah_stock, jumlah_beli, kode) values ('" + nama + "', " + harga + ", " + jumlah_stock + ", " + jumlah_beli + ", " + kode + ", " + id_jual + ")";
        System.out.println("\nExecuting query: " + query2);
        rset2 = stmt2.executeQuery(query2);
    }

    public ArrayList getAllPenjualan(int id_jual) throws SQLException {
        stmt2 = conn.createStatement();
        query2 = "select nama, harga, jumlah_stock, jumlah_beli, kode, id_jual from penjualan where id_jual = " + id_jual;
        System.out.println("\nExecuting query: " + query2);
        rset2 = stmt2.executeQuery(query2);
        while (rset2.next()) {
            arraySimpan2.add(new PenjualanData(rset2.getInt("KODE"), rset2.getInt("JUMLAH_STOCK"), rset2.getInt("JUMLAH_BELI"), rset2.getString("NAMA"), rset2.getDouble("HARGA"), rset2.getInt("ID_JUAL")));
        }
        return arraySimpan2;
    }
    ...
    public String getIdJualToString() throws SQLException {
        stmt2 = conn.createStatement();
        String idJual = null;
        query2 = "select max(id_jual) from penjualan";
        System.out.println("\nExecuting query: " + query2);
        rset2 = stmt2.executeQuery(query2);
        while (rset2.next()) {
            idJual = rset2.getString("max(id_jual)");
        }
        return idJual;
    }
    ...
    public static void main(String[] args) throws SQLException {
        ...
        dh.getDBConnection();
        System.out.println(dh.getIdJualToString());
    }
}
